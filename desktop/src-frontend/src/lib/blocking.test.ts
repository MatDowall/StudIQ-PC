...
/** The distinct row positions (plan arc-length along the run) present in a set of pieces. Rounded
 *  only enough to collapse float noise between bays, not enough to blunt an assertion. */
function rowPositions(pieces: { runPts: number }[]): number[] {
  return [...new Set(pieces.map((p) => Number(p.runPts.toFixed(9))))].sort((a, b) => a - b);
}
...
function topFace(m: { position: [number, number, number]; size: [number, number, number]; yaw: number; pitch: number }) {
  const n = rotate([0, 1, 0], m.yaw, m.pitch);

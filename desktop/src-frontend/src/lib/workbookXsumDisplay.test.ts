...
// child-reference form). Pure string logic. M5 suffixes: cost/Sub-Total → /S, rate → /R, qty → /Q.
...
const J = legacyColLetter(FIRST_USER_COL + 1);   // "J" (user col 2)
const K = legacyColLetter(FIRST_USER_COL + 2);   // "K" (user col 3)

describe("toDisplay: stored reference → clean positional", () => {
  it("XSUMTOT (cost /S) / RATE (/R) / QTY (/Q) drop the reference (keep dp)", () => {
    expect(toDisplay("=XSUMTOT(L1_sS6!H1:H1000)")).toBe("=XSUMTOT()");
    expect(toDisplay("=XSUMRATE(L1_sS6_sR2!H1:H1000,2)")).toBe("=XSUMRATE(2)");
    expect(toDisplay("=XSUMQTY(L1_sS6_sQ2!H1:H1000,3)")).toBe("=XSUMQTY(3)");
  });
  it("XSUMUSER family recover the user-column number from the reference column", () => {
    expect(toDisplay(`=XSUMUSER(L1_sS6!${J}1:${J}1000)`)).toBe("=XSUMUSER(2)");
    expect(toDisplay(`=XSUMRATEUSER(L1_sS6_sR2!${I}1:${I}1000,2)`)).toBe("=XSUMRATEUSER(1,2)");
    expect(toDisplay(`=XSUMRATEUSER(L1_sS6_sR2!${K}1:${K}1000)`)).toBe("=XSUMRATEUSER(3)");
  });
...
describe("toStored: clean positional → stored reference (from cell position)", () => {
  it("rebuilds the /S cost child for XSUMTOT and the /R rate child for XSUMRATE", () => {
    expect(toStored("=XSUMTOT()", "L1", 6)).toBe("=XSUMTOT(L1_sS6!H1:H1000)");
    expect(toStored("=XSUMRATE(2)", "L1/S6", 2)).toBe("=XSUMRATE(L1_sS6_sR2!H1:H1000,2)");
  });
  it("rebuilds the /Q child reference for the QTY family", () => {
    expect(toStored("=XSUMQTY(3)", "L1/S6", 2)).toBe("=XSUMQTY(L1_sS6_sQ2!H1:H1000,3)");
  });
  it("maps the user-column number back to the right column", () => {
    expect(toStored("=XSUMRATEUSER(1,2)", "L1/S6", 2)).toBe(`=XSUMRATEUSER(L1_sS6_sR2!${I}1:${I}1000,2)`);
    expect(toStored("=XSUMUSER(3)", "L1", 6)).toBe(`=XSUMUSER(L1_sS6!${K}1:${K}1000)`);
  });
  it("passes non-rollup content through unchanged", () => {
...
describe("round-trip", () => {
  it("stored → display → stored is stable for the same cell position", () => {
    const cases: Array<[string, string, number]> = [
      ["=XSUMTOT(L1_sS6!H1:H1000)", "L1", 6],
      ["=XSUMRATE(L1_sS6_sR2!H1:H1000,2)", "L1/S6", 2],
      ["=XSUMQTY(L1_sS6_sQ2!H1:H1000,3)", "L1/S6", 2],
      [`=XSUMRATEUSER(L1_sS6_sR2!${I}1:${I}1000,2)`, "L1/S6", 2],
      [`=XSUMUSER(L1_sS6!${K}1:${K}1000)`, "L1", 6],
    ];
...
describe("isStoredRollup", () => {
  it("recognises stored rollups only", () => {
    expect(isStoredRollup("=XSUMTOT(L1_sS6!H1:H1000)")).toBe(true);
